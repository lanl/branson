...
void comm_particles(std::vector<Cell> &cells, std::vector<Photon> &comm_particles) {
  std::cout<<"comm particles size: "<<comm_particles.size()<<std::endl;
...
std::vector<Photon> replicated_transport(Mesh &mesh,
  ...
  vector<Photon> boundary_wait_list;   //! Particles moving between cells (APEs)
  event_type event;
  std::vector<Cell> &cells = mesh.get_cells();

  uint64_t global_cycle_count = 0; //! Cycle count across all APEs

  while(std::any_of(cells.begin(), cells.end(), [] (const Cell &cell) {return cell.still_working();})) {

    uint64_t step_max_cycle_count = 0; //! Max cycles by an ape this transport phase
    ...
    for (auto &cell : cells) {
      ...
      std::cout<<"pparticle list size: "<<particle_list.size()<<std::endl;
    ...
    std::cout<<"Step max cycle count: "<<step_max_cycle_count<<" global cycle count: "<<global_cycle_count<<std::endl;
    // communicate boundary cells
    comm_particles(cells, boundary_wait_list);
  }
