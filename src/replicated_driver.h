...
void imc_replicated_driver(Mesh &mesh, IMC_State &imc_state,
  ...
  while (!imc_state.finished()) {
    ...
    // source all the particles, move them into the "particles" vector in each cell
    while(source) {
      Photon photon = source.get_photon(imc_state.get_rng(), imc_state.get_dt());
      auto &cell = mesh.get_cell_ref(photon.get_cell());
      cell.push_particle(photon);
    }
