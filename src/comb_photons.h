...
void comb_photons(std::vector<Photon> &census_photons, int64_t max_census_photons, RNG *rng) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, &global_census_E, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  for (auto ip = census_photons.begin(); ip != census_photons.end(); ++ip ) {
    ...
    // save the photon probabilistically or if there is only one photon remaining in the cell
  ...
  for (auto& p : post_comb_photons) {
    cell = p.get_cell();
    new_E = p.get_E() + (cell_census_E[cell]-cell_corrected_E[cell]) / double(cell_census_count[cell]);
    p.set_E(new_E);
  }
