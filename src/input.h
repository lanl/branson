...
public:
  ...
  Input(std::string fileName, const MPI_Types &mpi_types) {
    ...
    constexpr int n_bools = 3;
    constexpr int n_uint = 10;
    constexpr int n_doubles = 6;
    ...
    if (rank == 0) {
      ...
      // double
      vector<double> all_doubles = {t_start, dt,    t_finish,
                                    dt_mult,  dt_max, T_source};
      ...
      MPI_Bcast(&all_bools[0], n_bools, MPI_INT, 0, MPI_COMM_WORLD);
      use_comb = all_bools[0];
      print_verbose = all_bools[1];
      print_mesh_info = all_bools[2];
      ...
      MPI_Bcast(&all_doubles[0], n_doubles, MPI_DOUBLE, 0, MPI_COMM_WORLD);
      t_start = all_doubles[0];
      dt = all_doubles[1];
      t_finish = all_doubles[2];
      dt_mult = all_doubles[3];
      dt_max = all_doubles[4];
