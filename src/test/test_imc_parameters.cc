  ...
  // scope for MPI_Types
  {
    ...
    // file first
    {
      ...
      if (imc_parameters.get_dd_mode() != REPLICATED)
        simple_imc_parameters_pass = false;
      if(imc_parameters.get_use_gpu_transporter_bool() != false)
        simple_imc_parameters_pass = false;
