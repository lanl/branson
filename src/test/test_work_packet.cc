...
int main(void) {
  ...
  // test construction, get and set functions
  {
    ...
    if (work_packet.get_global_cell_ID() != 0)
      test_work_packet = false;
    if (work_packet.get_global_grip_ID() != 0)
      test_work_packet = false;
    if (work_packet.get_n_particles() != 0)
      test_work_packet = false;
    if (work_packet.get_create_E() != 0)
      test_work_packet = false;
    ...
    if (work_packet.get_global_cell_ID() != cell_ID)
      test_work_packet = false;
    if (work_packet.get_global_grip_ID() != grip_ID)
      test_work_packet = false;
    if (work_packet.get_n_particles() != n_particles)
      test_work_packet = false;
    if (work_packet.get_create_E() != emission_E)
      test_work_packet = false;
    for (uint32_t i = 0; i < 6; i++) {
      if (work_packet_coor[i] != cell_coor[i])
        test_work_packet = false;
    }
  ...
  //test split function
  {
    ...
    double e_leftover =
        double(n_particles - n_remain) / double(n_particles) * emission_E;
    ...
    if (big_work_packet.get_create_E() != e_remain)
      test_split_work_packet = false;
    for (uint32_t i = 0; i < 6; i++) {
      if (big_work_coor[i] != cell_coor[i])
        test_split_work_packet = false;
    }
    ...
    if (leftover_work.get_create_E() != e_leftover)
      test_split_work_packet = false;
    for (uint32_t i = 0; i < 6; i++) {
      if (leftover_work_coor[i] != cell_coor[i])
        test_split_work_packet = false;
    }
    ...
    if (test_split_work_packet)
      cout << "TEST PASSED: Split Work_Packet" << endl;
    else {
