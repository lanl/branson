...
int main (void) {
  ...
  // test to make sure rng only produces values in (0,1) and not [0,1] or (0,1] etc.
  {
    ...
    for(uint32_t i=0; i<n_sample;++i) {
      ...
      if (temp_random_double > max_rng) max_rng = temp_random_double;
      if (temp_random_double < min_rng) min_rng = temp_random_double;
    }
    ...
    if (max_rng >= 1.0 || max_rng <= 0.0) test_rng_max_min = false;
    if (min_rng >= 1.0 || min_rng <= 0.0) test_rng_max_min = false;
    if (!soft_equiv(random_avg, 0.5, 1.0e-3)) test_rng_max_min = false;

    if (test_rng_max_min) cout<<"TEST PASSED: RNG double in (0,1)"<<endl;
    else { 
  ...
  // test for identical seed and identical values
  {
    ...
    if (temp_random_double_1 !=  temp_random_double_2) test_rng_equal=false;
    ...
    if (test_rng_equal) 
      ...
      cout<<"TEST FAILED: RNG does not produce same value with same seed"<<endl;
      nfail++;
    }
  }
  ...
  // values
  {
    ...
    if (temp_random_double_1 ==  temp_random_double_2) test_rng_stream_equal=false;
  ...
  // test for identical seed and different values
  {
    ...
    if (temp_random_double_1 ==  temp_random_double_2) test_rng_diff=false;
