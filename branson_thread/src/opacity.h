...
class PL_Opacity : public Opacity
{
  PL_Opacity(void) {
    Opacity();
  }
  ...
  virtual double evaluate_opacity(const vector<double>& shape_vals, const vector<double>& mat_T) {
    check_mat_T(mat_T);
    check_shape_vals(shape_vals);
    double op1 = Opacity::get_opacty(mat_T[0]);
    double op2 = Opacity::get_opacty(mat_T[1]);
    return (op1*shape_vals[0] + op2*shape_vals[1]);
  }
